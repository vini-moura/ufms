...
summary(serie)
print(c(mean(serie), sd(serie)))
print(c(min(serie),max(serie)))
decomposicao.ad <- decompose(serie, type = )
decomposicao.ml <- decompose(serie, type="multiplicative")


ggplot(df, aes(x = data, y = serie)) +
  geom_line(colour = "#112446") +
  geom_line(y=decomposicao.ad$trend, colour = 'red')+
  labs(title="Empregos Formais No MS Com Tendência", y="Empregos")+
  theme_minimal()
...
# salvando graficos
png("decomposicao multi.png")
plot(decomposicao.ml)
dev.off()

png("decomposicao ad.png")
